...
program.  This program communicates with otp-enc.  It receives a plaintext
message and a key from otp-enc.  Then it encrypts the message using the key
and sends the encrypted message back to otp-enc.  This program cannot
communicate with otp-dec.
**********************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/******************************************************************
This is the encrypting function.  It encrypts the requested file
...
encryption method is shown on the following website:
https://en.wikipedia.org/wiki/One-time_pad
******************************************************************/
char* encriptText(char* text, char* key, char* output)
{
	int i;
	//char keyBuffer[1000];//buffer
	...
	for (i = 0; i < strlen(text); i++)
	{
		output[i] = (((text[i] - 64) + (key[i] - 64)) % 27) + 64;
...
int main(int argc, char *argv[])
{
	...
	char text[ARSIZE];//string to hold the message
	...
	// Set up the address struct for this process (the server) (taken from the example)
	...
	while (1)
	{
		...
		else
		{
			...
			else//decoding program attempted to connect (return an error)
			{
		...
		case 0://this is the child
		{
			...
			encriptText(text, key, output);//call the encripting function
			...
			charsRead = send(establishedConnectionFD, "##", 2, 0); // Write to the server that the message is over
